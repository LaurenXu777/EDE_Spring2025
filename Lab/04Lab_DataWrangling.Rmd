...
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake")) # %in% c is to checks whether each element in lakename is found in the vector c
...
# what code do you need to use, based on the class of the variable? -->1898
NTL.phys.data.Ex1.1 <- 
  filter(NTL.phys.data, year4 %in% c(1999))
NTL.phys.data.Ex1.2 <- 
  filter(NTL.phys.data, year4==1999)
NTL.phys.data.Ex1.3 <- NTL.phys.data %>%
  filter(year4==1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999. -->1971
NTL.phys.data.Ex2.1 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
NTL.phys.data.Ex2.2 <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake", year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Data will reorder after 
...
NTL.phys.data.Ex3.1 <- 
  NTL.phys.data %>%
  filter(lakename == "Tuesday Lake", year4 %in% c(1990:1999))  %>%
  mutate(Month = month(sampledate))%>% #mutate function is to create a new column, but we don't need that
  filter(Month==7)
